...
class cgol_pygame_animate :
    ...
    def report_mesure_elapse(self,val_str,itr):
       ...
       cpg = cyc_per_itr                        # Cycles per gen
       cpr = cyc_per_itr/self.rows              # Cycles per row
       cpe = cyc_per_itr/(self.rows*self.cols)  # Cycles per element
       
       rpt_file_name = 't%d/%s_RPT.csv' % (self.thread_id,self.pat_name) 
       print('CSV format report line written to %s' % rpt_file_name)       
       rpt_file = open(rpt_file_name,'w')
       rpt_file.write('%30s , %d , %d , %.2f  , %.2f  , %.2f\n' % (self.pat_name, itr, elps_cyc_cnt, cpg, cpr, cpe))
       rpt_file.close()

       all_rpt_file_name = 't0/cgol_all_rpt.csv'
       print('Updated performance history at %s' % all_rpt_file_name)
       echo_title_str = "echo  name , itr , cyc , cpg , cpr , cpe  > %s"   % all_rpt_file_name  
       print('%s' % echo_title_str)       
       os.system(echo_title_str)
       os.system('cat t0/*_h*.csv >> %s' % all_rpt_file_name)
